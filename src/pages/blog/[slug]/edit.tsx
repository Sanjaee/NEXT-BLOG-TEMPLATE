    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.data.sections && Array.isArray(result.data.sections)) {
          setSections(result.data.sections.map((s: Section) => ({
            id: s.id,
            type: s.type,
            title: s.title || '',
            content: s.content,
          ...
          setSections([{ type: 'text', title: '', content: '', order: 0 }]);
        }
      } else {
        router.push("/blog");
      }
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error("Gagal mengupdate post", {
          description: result.error || "Terjadi kesalahan saat mengupdate post.",
        });
      }
    } catch (error) {
      console.error("Error updating post:", error);
      toast.error("Gagal mengupdate post", {
        description: "Terjadi kesalahan saat mengupdate post. Silakan coba lagi.",
      });
    } finally {
      setSaving(false);
    }
